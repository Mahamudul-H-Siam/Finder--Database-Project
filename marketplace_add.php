...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        if ($stmt->execute()) {
            ...
            $adminStmt = $conn->prepare("SELECT UserID FROM USER WHERE Role = 'Admin'");
            $adminStmt->execute();
            $adminRes = $adminStmt->get_result();
            while ($admin = $adminRes->fetch_assoc()) {
                $notifStmt = $conn->prepare("INSERT INTO NOTIFICATION (UserID, Title, Message) VALUES (?, 'New Marketplace Item', ?)");
                $notifMsg = "A new item '$title' has been posted and is awaiting approval.";
...
<body>
    <div class="card">
        ...
        <?php if ($success): ?>
            <div class="success">Item posted successfully. It is pending admin approval and will be visible once approved.
            </div>
        <?php endif; ?>
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <textarea name="description" placeholder="Condition, size, location, pickup details"
                    required></textarea>
            </div>
            <div class="form-row">
                <div class="form-group">
